...
def render_login_page():
    ...
    with col2: # 로그인 폼이 들어갈 중앙 컬럼
        ...
        empty_left_sub_col, img_col, title_col = st.columns([0.1, 0.5, 0.9) # 비율 조정: 0.2만큼 왼쪽으로 이동
        
        with img_col:
            if os.path.exists(img_path):
                st.image(img_path, width=500, use_container_width=False) # 이미지 크기 조정
            else:
        ...
        with title_col:
            ...
            st.markdown('<h2 style="margin-bottom: 1.5rem; margin-top: 2rem;">로봇수술 이해쏙쏙</h2>', unsafe_allow_html=True)
